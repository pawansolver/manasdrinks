...
export default function FlyingPosters({
  ...
  // ✅ Preload images before creating WebGL scene
  useEffect(() => {
    if (!items.length) return;
    let isCancelled = false;

    const preloadImages = items.map(src => {
  ...
  // ✅ Only initialize WebGL once images are preloaded
  useEffect(() => {
  ...
  }, [isLoaded, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ]);
  ...
  return (
    <div ref={containerRef} className={`posters-container ${className}`} {...props}>
      {/* Show subtle loading fade-in */}
      <canvas
        ref={canvasRef}
        className={`posters-canvas transition-opacity duration-700 ${
          isLoaded ? "opacity-100" : "opacity-0"
        }`}
      />
      {!isLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-black text-white text-lg font-semibold">
          Loading Posters...
        </div>
      )}
    </div>
