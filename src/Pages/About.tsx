...
const PremiumBackground = () => {
  ...
  const iceCubes = Array.from({ length: 12 }).map((_, i) => ({
    id: i,
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none bg-[#020202]">
      <div className="absolute top-[-30%] left-[-10%] w-[70%] h-[70%] rounded-full bg-gradient-radial from-emerald-600/15 via-transparent to-transparent blur-[140px]" />
      <div className="absolute bottom-[-30%] right-[-10%] w-[70%] h-[70%] rounded-full bg-gradient-radial from-cyan-700/20 via-transparent to-transparent blur-[140px]" />
      <div className="absolute top-[40%] left-[30%] w-[40%] h-[40%] rounded-full bg-gradient-radial from-blue-900/30 via-transparent to-transparent blur-[160px]" />

      {bubbles.map((bubble) => (
        <motion.div
          key={`bubble-${bubble.id}`}
          className="absolute bottom-[-10%] rounded-full bg-gradient-to-tr from-white/10 to-transparent backdrop-blur-[1px] border border-white/20 shadow-[inset_0_2px_8px_rgba(255,255,255,0.4)]"
          style={{ width: bubble.size, height: bubble.size, left: `${bubble.left}%` }}
          animate={{
            y: ["0vh", "-120vh"],
            x: ["0px", `${bubble.wobble}px`, "0px", `${-bubble.wobble}px`, "0px"],
          }}
          transition={{
      ...
      <div className="absolute inset-0" style={{ perspective: '1000px' }}>
        {iceCubes.map((cube) => (
          <motion.div
            key={`cube-${cube.id}`}
            className="absolute rounded-3xl bg-gradient-to-br from-white/20 to-white/5 backdrop-blur-md border border-white/30 shadow-[0_15px_35px_rgba(0,0,0,0.5),inset_0_4px_10px_rgba(255,255,255,0.6)]"
            style={{ width: cube.size, height: cube.size, left: `${cube.left}%`, top: `${cube.top}%` }}
            animate={{
              y: ["0%", "-30%", "10%", "0%"],
              x: ["0%", "10%", "-10%", "0%"],
              rotateX: [cube.rotationX, cube.rotationX + 180, cube.rotationX + 360],
              rotateY: [cube.rotationY, cube.rotationY + 180, cube.rotationY + 360],
              rotateZ: [0, 180, 360],
            }}
            transition={{ duration: cube.duration, repeat: Infinity, ease: "linear", delay: cube.delay }}
          >
            <div className="absolute inset-2 rounded-2xl bg-gradient-to-tl from-transparent to-white/10 border border-white/10" />
          </motion.div>
        ))}
      </div>
    </div>
              ...
              {timelineEvents.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`bg-white/5 backdrop-blur-xl border border-white/10 border-t-4 ${brandClass} rounded-xl p-6 flex flex-col h-full hover:bg-white/10 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(0,0,0,0.6)] shadow-[0_8px_32px_rgba(0,0,0,0.4)]`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {culturalImpacts.map((item, index) => (
                <motion.div
                  ...
                  viewport={{ once: true, amount: 0.5 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  className="bg-white/5 border border-white/10 p-6 rounded-2xl text-center backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(0,0,0,0.6)] hover:bg-white/10 transition-all duration-300"
                >
